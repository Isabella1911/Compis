...
class Tema:
    ...
    BG_PANEL    = "#252526"   # Paneles
    BG_PANEL_2  = "#2d2d30"   # Paneles secundarios (gutter, headers)
    BG_BUTTON   = "#3c3c3c"   # Botones
    BG_BUTTON_H = "#505050"   # Botones hover
    BG_ACTIVE   = "#094771"   # Selección / tab activo
    BORDE       = "#3c3c3c"
    ...
    ACCENT      = "#569cd6"   # Azul VSCode
    ACCENT_DARK = "#0e639c"

    OK          = "#6bbf59"   # Verde
    WARN        = "#dcdcaa"   # Amarillo
    ERR         = "#f48771"   # Rojo
    INFO        = "#569cd6"   # Azul info
    ...
    CURRENT_LINE  = "#2a2d2e"   # Fondo de la línea actual
    MATCH_BRACKET = "#264f78"   # Fondo del bracket pareado
    DND_HINT      = "#0e639c"   # Estado del toolbar mientras se arrastra
...
class IDE:
    ...
    def _configurar_estilo(self):
        ...
        s.configure("Status.TLabel", background=Tema.BG_PANEL_2,
                    foreground=Tema.FG_MUTED, padding=(8, 2))
        s.configure("Accent.TLabel", background=Tema.BG,
        ...
        s.configure("TNotebook.Tab",
                    ...
                    font=fuente_ui)
        s.map("TNotebook.Tab",
              background=[("selected", Tema.BG),
                           ("active", Tema.BG_PANEL_2)],
        ...
        # Separator
        s.configure("TSeparator", background=Tema.BORDE)
    ...
    def _construir_toolbar(self):
        barra = ttk.Frame(self.root, style="Toolbar.TFrame", padding=6)
        barra.pack(side="top", fill="x")

        # Acciones principales (4)
        ttk.Button(barra, text="▶  Compilar",
                    style="Accent.TButton",
                    command=self.compilar).pack(side="left", padx=(4, 6))
        ttk.Button(barra, text="▷  Ejecutar",
                    style="Accent.TButton",
                    command=self.ejecutar).pack(side="left", padx=4)

        ttk.Separator(barra, orient="vertical").pack(
            side="left", fill="y", padx=8)

        ttk.Button(barra, text="⟳  LR(0)",
                    command=self.renderizar_lr0).pack(side="left", padx=4)
        ttk.Button(barra, text="✕  Limpiar",
                    command=self.limpiar_salida).pack(side="left", padx=4)
        ...
        self._toolbar_estado.pack(side="right", padx=8)
    ...
    def _construir_cuerpo(self):
        ...
        paned_h.pack(fill="both", expand=True, padx=4, pady=4)
    ...
    def _construir_sidebar(self, paned):
        ...
        ttk.Label(side, text="Proyecto", style="Side.TLabel",
                   padding=(10, 8, 4, 4)).pack(anchor="w", fill="x")

        self._proyecto = ttk.Treeview(
            side, show="tree", selectmode="browse",
            style="Proyecto.Treeview")
        self._proyecto.pack(fill="both", expand=True, padx=4, pady=(0, 8))
    ...
    def _construir_editor(self, paned):
        marco = ttk.Frame(paned, style="Editor.TFrame")
    ...
    def _construir_resultados(self, paned):
        marco = ttk.Frame(paned, style="Card.TFrame")
        ...
        nb1 = ttk.Notebook(g1)
        nb1.pack(fill="both", expand=True, padx=4, pady=4)
        for clave, titulo in (
            ("salida",     "Consola"),
            ("tokens",     "Tokens"),
        ...
        nb3 = ttk.Notebook(g3)
        nb3.pack(fill="both", expand=True, padx=4, pady=4)
    ...
    def _panel_tabla(self, notebook, titulo):
        ...
        tabla.pack(fill="both", expand=True, padx=2, pady=2)
    ...
    def _construir_panel_lr0(self, notebook):
        ...
        cabecera = ttk.Frame(frame, style="Card.TFrame", padding=(6, 6))
    ...
    def _construir_barra_estado(self):
        ...
        contenido = ttk.Frame(barra, style="Status.TFrame", padding=(4, 0))
