...
void generar_analizador_lexico(const ArchivoYalex& yalex, AFD& afd_min, const std::string& nombre_salida) {
    std::ofstream out("output/" + nombre_salida + ".cpp");
...
// ╔══════════════════════════════════════════════════════════════╗
// ║  SECCIÓN 15 — EJECUTAR LEXER EN MEMORIA                    ║
// ║  Corre el AFD minimizado directamente sobre un string.     ║
// ║  Usada por main.cpp (orquestador completo).                ║
// ╚══════════════════════════════════════════════════════════════╝

ResultadoLexico ejecutar_lexer(AFD& afd_min, const ArchivoYalex& yalex, const std::string& entrada) {
    std::vector<std::string> syms(afd_min.alfabeto.begin(), afd_min.alfabeto.end());
    std::map<std::string, int> si;
    for (size_t i = 0; i < syms.size(); i++) si[syms[i]] = (int)i;

    int num_estados  = (int)afd_min.estados.size();
    int num_simbolos = (int)syms.size();
    int estado_ini   = afd_min.estado_inicial->id;

    std::vector<int> char_to_sym(256, -1);
    for (size_t i = 0; i < syms.size(); i++)
        if (syms[i].size() == 1)
            char_to_sym[(unsigned char)syms[i][0]] = (int)i;

    std::vector<std::vector<int>> transicion(num_estados, std::vector<int>(num_simbolos, -1));
    for (auto& up : afd_min.estados) {
        auto* e = up.get();
        for (auto& [s, d] : e->transiciones) {
            auto it = si.find(s);
            if (it != si.end())
                transicion[e->id][it->second] = d->id;
        }
    }

    std::vector<int> token_aceptado(num_estados, -1);
    for (auto& up : afd_min.estados) {
        auto* e = up.get();
        if (e->es_final && e->token_id >= 0)
            token_aceptado[e->id] = e->token_id;
    }

    std::vector<std::string> token_names, token_actions;
    for (auto& r : yalex.reglas) {
        token_names.push_back(r.nombre_token);
        token_actions.push_back(r.accion);
    }
    ...
    while (pos < entrada.size()) {
        ...
        while (i < entrada.size()) {
            int sy = char_to_sym[(unsigned char)entrada[i]];
            if (sy < 0) break;
            int sg = (sy < num_simbolos) ? transicion[ea][sy] : -1;
            if (sg < 0) break;
            ea = sg; i++;
            if (token_aceptado[ea] >= 0) { uap = i; ut = token_aceptado[ea]; }
        }
...
#endif // COMPILAR_CON_ORQUESTADOR